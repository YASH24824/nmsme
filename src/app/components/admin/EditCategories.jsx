...
const EditCategoryForm = ({
  ...
  useEffect(() => {
    if (category) {
      ...
      setCurrentImage(category.icon || null);
    }
  }, [category]);
  ...
  if (!showModal || !category) return null;
  ...
  const getAvailableParentCategories = () => {
    return categories.filter((cat) => {
      ...
      // Can't be parent if it's already a child of current category
      const isChildOfCurrent = (checkCategory) => {
        if (!checkCategory.parent_category_id) return false;
        if (checkCategory.parent_category_id === category.category_id)
          return true;
